...
pub fn withdraw(ctx:Context<WithdrawCtx> , cid:u64 , amount:u64)->Result<()>{
    ...
    let min_rent_balance = Rent::get()?.minimum_balance(campaign.to_account_info().data_len());
    if amount> (**campaign.to_account_info().lamports.borrow() - min_rent_balance )  {
    ...
    let creators_amount = amount - platform_fee;

    **campaign.to_account_info().try_borrow_mut_lamports()? -= creators_amount;
    **creator.to_account_info().try_borrow_mut_lamports()? += creators_amount;
...
pub struct WithdrawCtx<'info>{
    ...
    /// CHECK: Should pass the owner's address who intially deployed the contract
    #[account(mut)]
